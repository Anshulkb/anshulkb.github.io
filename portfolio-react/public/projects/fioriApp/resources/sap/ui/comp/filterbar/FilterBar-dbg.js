...
sap.ui.define([
	...
	FilterBarLayoutMixin.call(FilterBar.prototype, {
		_getUseToolbar: function () {
			return this.getUseToolbar();
		},
		_getAdvancedMode: function () {
			return this.getAdvancedMode();
		},
		_shouldRenderAdvancedLayout: function () {
			return this.getAdvancedMode() && !this.getIsRunningInValueHelpDialog();
		},
		_getFilterBarExpanded: function () {
			return this.getFilterBarExpanded();
		},
		_getFilterItems: function () {
			return this._mAdvancedAreaFilterFlat.length ? this._mAdvancedAreaFilterFlat : this._getAllFilterItemsFlat();
		},
		_isFilterItemHidden: function (oItem) {
		...
		_shouldSkipRenderFilters: function (index) {
			return this.getIsRunningInValueHelpDialog() && this._nMaxFiltersByDefault === index && !this.getShowAllFilters();
		},
		_shouldSkipRenderButtons: function () {
			return this._getUseToolbar();
		},
	...
	FilterBar.prototype._suspendSuggestionBindingOnhiddenItems = function() {
		this.getFilterGroupItems().forEach((oItem) => {
			...
			const oControlBinding = oControl.getBinding(sAggregationName);

			if (!oControlBinding) {
				return;
			}
			if (oItem.getVisible() && oControlBinding.isSuspended()) {
				oControlBinding.resume();
			} else if (!oItem.getVisible() && !oControlBinding.isSuspended()) {
				oControlBinding.suspend();
			}
		});
	};
